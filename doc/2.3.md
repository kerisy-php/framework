...
所有方法参考 Kerisy\Foundation\Storage\Adapter\SQLAbstract 类
...
```
    $this->insert(['name'=>'kerisy']);
    //or
    $this->insert(['name'=>'kerisy'],'user');
```
* insertIf(array $data, array $where, $tableName = null) 根据条件，有则更新，无则插入
```
    $this->insertIf(['name'=>'kerisy'],['id'=>1]);
    //or
    $this->insertIf(['name'=>'kerisy'],['id'=>1],'user');
```
...
* field($fieldStr) 设置查询的字段

#####where 条件查询
* =

```
    $this->get("name"=>"kerisy");
    //相当于 WHERE name = 'kerisy'
```
* in 查询
```
    $this->get("id"=>["in",[1,2]]);
...
* 单个字段多种查询条件
```
    $this->get("id"=>[["in",[1,2],"OR"],[">=",2,"OR"]],"name"=>"kerisy");
    //相当于 WHERE id in(1,2) OR id >=2 OR name='kerisy'
```
