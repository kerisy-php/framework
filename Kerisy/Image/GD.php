...
class Kerisy_Image_GD extends Kerisy_Image {
	...
	public static function check()
	{
		...
		{
			show_error('GD is either not installed or not enabled, check your configuration');
		}

		if (defined('GD_BUNDLED'))
		{
			// Get the version via a constant, available in PHP 5.
			Kerisy_Image_GD::$_bundled = GD_BUNDLED;
		}
		else
		{
			...
			Kerisy_Image_GD::$_bundled = (bool) preg_match('/\bbundled\b/i', $info['GD Version']);
		}
		...
		{
			show_error('Kerisy_Image_GD requires GD 2.0 or greater, you have ' . $version);
		}

		return Kerisy_Image_GD::$_checked = TRUE;
	}
	...
	{
		if ( ! Kerisy_Image_GD::$_checked)
		{
			// Run the install check
			Kerisy_Image_GD::check();
		}
		...
		{
			show_error('Installed GD does not support :type images');
		}
	...
	protected function _do_crop($width, $height, $offset_x, $offset_y)
	{
		...
		if (imagecopyresampled($image, $this->_image, 0, 0, 0, 0, $width, $height, $width, $height))
		{
			// Swap the new image for the old one
			imagedestroy($this->_image);
			$this->_image = $image;
	...
	{
		if ( ! Kerisy_Image_GD::$_bundled)
		{
			show_error('This method requires :function, which is only available in the bundled version of GD');
		}
