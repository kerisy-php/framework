...
abstract class Kerisy_Image_Abstract {
	 ...
	 */
	public static $default_driver = 'ImageMagick';
	 ...
	 *
	 *     $image = Kerisy_Image::factory('upload/test.jpg');
	 *
	 * @param   string   image file path
	 * @param   string   driver type: GD, ImageMagick, etc
	 * @return  Image
	 * @uses    Kerisy_Image::$default_driver
	 */
	public static function factory($file, $driver = NULL)
	{
		if ($driver === NULL)
		{
			...
			$driver = Kerisy_Image::$default_driver;
		}

		// Set the class name
		$class = 'Kerisy_Image_'.$driver;
    Kerisy::import('Kerisy.Image.'.$driver);
	...
	public function __construct($file)
	{
		...
		{
			show_error('Not an image or invalid image: :file');
		}
	 ...
	 *     $image->resize(200, 200, Kerisy_Image::INVERSE);
	 *
	...
	public function resize($width = NULL, $height = NULL, $master = NULL)
	{
		if ($master === NULL)
		{
			...
			$master = Kerisy_Image::AUTO;
		}
		// Kerisy_Image::WIDTH and Kerisy_Image::HEIGHT deprecated. You can use it in old projects,
		// but in new you must pass empty value for non-master dimension
		elseif ($master == Kerisy_Image::WIDTH AND ! empty($width))
		{
			$master = Kerisy_Image::AUTO;
		...
		elseif ($master == Kerisy_Image::HEIGHT AND ! empty($height))
		{
			$master = Kerisy_Image::AUTO;
		...
		if (empty($height))
		{
			...
			else
			{
				...
				$master = Kerisy_Image::WIDTH;
			}
		}

		switch ($master)
		{
			case Kerisy_Image::AUTO:
				// Choose direction with the greatest reduction ratio
				$master = ($this->width / $width) > ($this->height / $height) ? Kerisy_Image::WIDTH : Kerisy_Image::HEIGHT;
			break;
			case Kerisy_Image::INVERSE:
				// Choose direction with the minimum reduction ratio
				$master = ($this->width / $width) > ($this->height / $height) ? Kerisy_Image::HEIGHT : Kerisy_Image::WIDTH;
			break;
		}

		switch ($master)
		{
			case Kerisy_Image::WIDTH:
				// Recalculate the height based on the width proportions
				$height = $this->height * $width / $this->width;
			break;
			case Kerisy_Image::HEIGHT:
	 ...
	 *
	 * @param   integer  direction: Kerisy_Image::HORIZONTAL, Kerisy_Image::VERTICAL
	 * @return  $this
	 * @uses    Kerisy_Image::_do_flip
	 */
	public function flip($direction)
	{
		if ($direction !== Kerisy_Image::HORIZONTAL)
		{
			// Flip vertically
			$direction = Kerisy_Image::VERTICAL;
		}
	 ...
	 * @return  $this
	 * @uses    Kerisy_Image::_do_reflection
	 */
	 ...
	 *     $mark = Kerisy_Image::factory('upload/watermark.png');
	 ...
	 * @return  $this
	 * @uses    Kerisy_Image::_do_watermark
	 */
	 ...
	 * @return  $this
	 * @uses    Kerisy_Image::_do_background
	 */
	...
	public function save($file = NULL, $quality = 100)
	{
		...
		if (is_file($file))
		{
			if ( ! is_writable($file))
			{
				show_error('File must be writable: :file');
			}
		}
		else
		{
			...
			{
				show_error('Directory must be writable: :directory');
			}
		}
