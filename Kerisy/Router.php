...
class Kerisy_Router
{
    ...
    public function routing()
    {
        ...
        // 如果
        if (!$this->_uri->useControllerPrefix()) {
    ...
    protected function initRoutes()
    {
        ...
        foreach ($this->_modules as $module) {
            $routes = Kerisy::config($module)->get('routes')->routes;
...
//             $params = array_combine($matches[1], $matches[2]);
    ...
    protected function setController($controller)
    {
        if ($controller) {
            ...
            $this->_controller = ucfirst($this->getModuleDefaultController($this->getControllerDirectory()));
        }
    }

    protected function getModuleDefaultController($type)
    {
        ...
        try {
            ...
            return $tmp_array[strtolower($type) . '_default_controller'];
        } catch (Exception $e) {
    ...
    protected function setAction($action)
    {
        if ($action) {
            ...
            $controller_class = $this->_module . '_Controller_' . ($this->getControllerDirectory() ? ($this->getControllerDirectory() . '_') : '') . $this->_controller;
            $default_action = Kerisy::controller($controller_class)->getDefaultAction();
            $this->_action = ucfirst($default_action);
    ...
    {
        return $this->_uri->useControllerPrefix() ? $this->_uri->getControllerPrefix() : '';
    }
}
