...
class Kerisy_Controller {
  ...
  protected $_header_info = "Content-type:text/html; charset=utf-8";
  ...
  private function initView() {
	$template_dir = $this->_template_dir ? $this->_template_dir : (APP_PATH . 'templates' . DS . 'front' . DS . 'default' . DS);
  ...
  public function redirect($uri) {
	if (strstr($uri, '://')) {
	  ...
	  header('Location: ' . $this->baseUrl() . '/' . ltrim($uri, '/'));
	}
	exit;
  }
