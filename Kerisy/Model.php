...
abstract class Kerisy_Model
{
    ...
    {
        if (!$this->_db) {
            $this->_db = Kerisy::useClass('Kerisy_Db')->getDb();
        }
...
//        pe($select->__toString());
        return $this->slave()->fetchAll($select);
    }

    public function fetchAllWithSelect(
        $selectParms,
        $limit = 10,
        $offset = 0,
        $where = null,
        ...
        $select = $this->select();
        $select->from($this->getTableFullName(), $selectParms ?: '*');
...
//        pe($select->__toString());
    ...
    public function fetchRowWithSelect($selectParms, $where = null, $order = null)
    {
        $select = $this->select();
        $select->from($this->getTableFullName(), $selectParms);
